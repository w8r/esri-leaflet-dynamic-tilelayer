describe('L.esri.Layers.TiledDynamicMapLayer', function() {
	...
	beforeEach(function() {
		...
		layer = L.esri.Layers.tiledDynamicMapLayer({
			url: url,
			f: 'json'
		});
		map = createMap();
	...
	it('should have a L.esri.Layers.tiledDynamicMapLayer alias', function() {
		expect(L.esri.Layers.tiledDynamicMapLayer({
			url: url
		})).to.be.instanceof(L.esri.Layers.TiledDynamicMapLayer);
	});

	it('should be a subclass of L.TileLayer', function() {
		expect(L.esri.Layers.tiledDynamicMapLayer({
			url: url
		})).to.be.instanceof(L.TileLayer);
	});
